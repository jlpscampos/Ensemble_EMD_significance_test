...
y2=filterfft(f, y, cutoff_freq, my_freqs, opt); % filter amplitudes
...
function y2=filterfft(f, y, cutoff, wins, opt)
nf=length(f);
ny=length(y);
if ~(ny/2+1 == nf),
...
y2=zeros(1,ny);
if ~isempty(cutoff)
    if(opt == 1) ; ind1=find(f<=cutoff); end
    if(opt == 0) ; ind1=find(f>=cutoff); end
